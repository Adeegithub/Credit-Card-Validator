...
// Function to calculate checksum and validate the credit card number
bool validateCreditCard(int sum1, int sum2, int lastDigit) {
    int total = sum1 + sum2;
    ...
    if (checksum == lastDigit) {
        cout << "Check sum " << checksum << " and last digit " << lastDigit << " are same: Valid Credit Card Number" << endl;
        return true;
    } else {
        cout << "Check sum " << checksum << " and last digit " << lastDigit << " are not same: Invalid Credit Card Number" << endl;
        return false;
    }
}
...
int main() {
    ...
    int lastDigit = arr[count - 1];

    // Validate the credit card number
    validateCreditCard(sum1, sum2, lastDigit);
